        ...
        if(itemsPregen) {
            ...
            for(pos = start; pos < end; pos++) {
                ...
                var processId = function(full, prop, id) {
                    ...
                    if (id.indexOf('_') <= 0="" &&="" id.indexof('p')="=" -1)="" ids.push(elementid);="" return="" prop="" +="" '="' + elementId + '" ';="" };="" var="" copy="(useAlt" ?="" altdiv="" :="" div).clone();="" usealt="!useAlt;" copy.attr('id',="" itemelementid);="" copy.html(div.html().replace(="" (id|for)="?u([0-9]+(p([0-9]){3})?(_[_a-z0-9]*)?)" g,="" processid));="" copy.css({="" 'position':="" 'absolute',="" 'top':="" top="" 'px',="" 'left':="" left="" 'width':="" obj.width="" 'height':="" obj.height="" 'px'="" });="" $('#'="" repeaterid).append(copy);="" i++;="" if(wrap="" !="-1" i="" %="" wrap="=" 0)="" {="" if(vertical)="" }="" else="" if="" (vertical)="" showncount="end" -="" start;="" repeatersize="{" width:="" paddingx,="" height:="" paddingy};="" if(showncount=""> 0) {
...
//        }
    ...
    //        if(wrap == -1 || itemCount <= wrap)="" {="" if(vertical)="" height="" +="heightIncrement" *="" (itemcount="" -="" 1);="" else="" width="" }="" var="" primarydim="wrap;" secondarydim="Math.ceil(itemCount" primarydim);="" (primarydim="" (secondarydim="" return="" width:="" width,="" height:="" };="" _repeatermanager.getrepeatersize="_getRepeaterSize;" });="" *******="" dynamic="" panel="" manager="" ********="" $axure.internal(function($ax)="" todo:="" probably="" a="" lot="" of="" the="" functions="" from="" pagescript="" should="" be="" moved="" here="" at="" some="" point...="" _dynamicpanelmanager="$ax.dynamicPanelManager" =="" {};="" _isidfittocontent="_dynamicPanelManager.isIdFitToContent" function(id)="" obj="$obj(id);" if="" (!obj="" ||="" !$ax.public.fn.isdynamicpanel(obj.type)="" !obj.fittocontent)="" false;="" jobj="$jobj($ax.visibility.GetPanelState(id));" jobj.css('position')="=" 'relative';="" _fitparentpanel="function(widgetId)" find="" parent="" there="" is="" one.="" parentpanelinfo="getParentPanel(widgetId);" if(parentpanelinfo)="" parentid="parentPanelInfo.parent;" if(_updatefitpanel(parentid,="" parentpanelinfo.state))="" _fitparentpanel(parentid);="" return;="" otherwise,="" try="" to="" get="" repeater="" parentrepeaterid="$ax.getParentRepeaterFromElementId(widgetId);" repeaterobj="$obj(parentRepeaterId);" if(!repeaterobj="" widgetid="=" !repeaterobj.repeaterpropmap.fittocontent)="" itemid="$ax.repeater.getItemIdFromElementId(widgetId);" size="getContainerSize($ax.repeater.createElementId(parentRepeaterId," itemid));="" if($ax.repeater.setitemsize(parentrepeaterid,="" itemid,="" size.width,="" size.height))="" _fitparentpanel(parentrepeaterid);="" _dynamicpanelmanager.fitparentpanel="_fitParentPanel;" _dynamicpanelmanager.initialize="function()" _dynamicpanelmanager.initfitpanels($ax('*'));="" $axure.resize(_handleresize);="" _dynamicpanelmanager.initfitpanels="function(query)" fittocontent="[];" query.each(function="" (obj,="" elementid)="" scriptid="$ax.repeater.getScriptIdFromElementId(elementId);" if($ax.public.fn.isdynamicpanel(obj.type)="" &&="" obj.fittocontent="" !$ax.visibility.isscriptidlimbo(scriptid))="" fittocontent[fittocontent.length]="elementId;" for(var="" i="fitToContent.length" 1;="">= 0; i--) {
